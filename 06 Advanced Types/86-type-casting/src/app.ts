...
const userInputElement = document.getElementById(
  'user-input'
) as HTMLInputElement; // we can also add something after the thing we want to type cast, so after this selection in this case, and that thing is the "as" keyword. And then here we again add the type to which we wanna cast this (HTMLInputElement). So, this now tells TypeScript that the expression in front of it, in this case, this expression here, will yield a value of type HTMLInputElement. Therefore, again, we get no error.
userInputElement.value = 'Hello there!';
